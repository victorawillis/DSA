 ...
 *previous points to item in front of it (eventually to the top)
*/
...
disk *pop(disk **head)
{
	...
	// Save the head before we change it (single deref since we have head's address)
	disk *ghost_head = *head;
	// Element before the head will have it's "prev" point to NULL
	/*
	disk *current = *head;
	...
	*head = ghost_head->next;
	// THIS DETACHES THE OLD HEAD FROM THE LIST
	ghost_head->next = NULL;
	// Because we called the head by reference, we won't return the new head
	//
	// We will however return the previous head
	return ghost_head;
}
